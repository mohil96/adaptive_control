...
%         f_add=f_add+alpha*(phi')*phi;
%         theta_add=theta_add+alpha*phi*y;
%         y2=y1;
%         y1=y;
%         u2=u1;
%         u1=1;
...
%   ct2=input('  :  ');
  ...
  for k=1:kn,
    ...
    if lam2==0,
        F=(F-F*phi*phi'*F/(lam1+phi'*F*phi))/lam1;
    end
  ...
  figure;
  subplot(211)
  plot(t,err); title('prediction error, series parallel predictor');
  subplot(212)
